...
def validate_field_name(bases: List[Type['BaseModel']], field_name: str) -> None:
    """
    Ensure that the field's name does not shadow an existing attribute of the models.
    """
    for base in bases:
...
def generate_model_signature(
    ...
    """
    Generate signature for models based on its fields
    """
    ...
    if var_kw and use_var_kw:
        ...
        if [(p.name, p.kind) for p in present_params] == default_model_signature:
            # if this is the standard models signature, use extra_data as the extra args name
...
class GetterDict(Representation):
    ...
    def extra_keys(self) -> Set[Any]:
        """
        We don't want to get any other attributes of obj if the models didn't explicitly ask for them
        """
        return set()
...
def get_discriminator_alias_and_values(tp: Any, discriminator_key: str) -> Tuple[str, Tuple[str, ...]]:
    ...
    if is_union(get_origin(tp)):
        ...
        if not is_literal_type(t_discriminator_type):
            raise ConfigError(f'Field {discriminator_key!r} of models {tp.__name__!r} needs to be a `Literal`')
