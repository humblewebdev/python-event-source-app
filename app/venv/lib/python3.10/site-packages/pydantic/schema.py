...
default_ref_template = '#/definitions/{models}'
...
def schema(
    ...
    *,
    ...
    :param ref_template: Use a ``string.format()`` template for ``$ref`` instead of a prefix. This can be useful
      ...
      a sibling json file in a ``/schemas`` directory use ``"/schemas/${models}.json#"``.
...
def model_schema(
    ...
    """
    Generate a JSON Schema for one models. With all the sub-models defined in the ``definitions`` top-level
    JSON key.

    :param model: a Pydantic models (a class that inherits from BaseModel)
    :param by_alias: generate the schemas using the aliases defined, if any
    :param ref_prefix: the JSON Pointer prefix for schema references with ``$ref``, if None, will be set to the
      default of ``#/definitions/``. Update it if you want the schemas to reference the definitions somewhere
      else, e.g. for OpenAPI use ``#/components/schemas/``. The resulting generated schemas will still be at the
    ...
    :param ref_template: Use a ``string.format()`` template for ``$ref`` instead of a prefix. This can be useful for
      ...
      sibling json file in a ``/schemas`` directory use ``"/schemas/${models}.json#"``.
    :return: dict with the JSON Schema for the passed ``models``
    """
...
def field_schema(
    ...
    *,
    ...
    is a models and has sub-models, and those sub-models don't have overrides (as ``title``, ``default``, etc), they
...
def get_flat_models_from_model(model: Type['BaseModel'], known_models: TypeModelSet = None) -> TypeModelSet:
    """
    Take a single ``models`` and generate a set with itself and all the sub-models in the tree. I.e. if you pass
    models ``Foo`` (subclass of Pydantic ``BaseModel``) as ``models``, and it has a field of type ``Bar`` (also
    subclass of ``BaseModel``) and that models ``Bar`` has a field of type ``Baz`` (also subclass of ``BaseModel``),
    the return value will be ``set([Foo, Bar, Baz])``.

    :param model: a Pydantic ``BaseModel`` subclass
    :param known_models: used to solve circular references
    :return: a set with the initial models and all its sub-models
    """
    known_models = known_models or set()
...
def get_flat_models_from_field(field: ModelField, known_models: TypeModelSet) -> TypeModelSet:
    """
    Take a single Pydantic ``ModelField`` (from a models) that could have been declared as a sublcass of BaseModel
    (so, it could be a submodel), and generate a set with its models and all the sub-models in the tree.
    I.e. if you pass a field that was declared to be of type ``Foo`` (subclass of BaseModel) as ``field``, and that
    models ``Foo`` has a field of type ``Bar`` (also subclass of ``BaseModel``) and that models ``Bar`` has a field of
    type ``Baz`` (also subclass of ``BaseModel``), the return value will be ``set([Foo, Bar, Baz])``.

    :param field: a Pydantic ``ModelField``
    :param known_models: used to solve circular references
    :return: a set with the models used in the declaration for this field, if any, and all its sub-models
    """
    from .main import BaseModel
...
def get_flat_models_from_fields(fields: Sequence[ModelField], known_models: TypeModelSet) -> TypeModelSet:
    """
    Take a list of Pydantic  ``ModelField``s (from a models) that could have been declared as subclasses of ``BaseModel``
    (so, any of them could be a submodel), and generate a set with their models and all the sub-models in the tree.
    I.e. if you pass a the fields of a models ``Foo`` (subclass of ``BaseModel``) as ``fields``, and on of them has a
    field of type ``Bar`` (also subclass of ``BaseModel``) and that models ``Bar`` has a field of type ``Baz`` (also
    subclass of ``BaseModel``), the return value will be ``set([Foo, Bar, Baz])``.

    :param fields: a list of Pydantic ``ModelField``s
    :param known_models: used to solve circular references
    :return: a set with any models declared in the fields, and all their sub-models
    """
    flat_models: TypeModelSet = set()
    for field in fields:
...
def model_process_schema(
    ...
    *,
    ...
    Take a single ``models`` and generate its schema. Also return additional schema definitions, from sub-models. The
...
def model_type_schema(
    ...
    *,
    ...
    Take a single ``models`` and generate the schema for its type only, not including additional
    information as title, etc. Also return additional schema definitions, from sub-models.
    """
    properties = {}
    required = []
...
def normalize_name(name: str) -> str:
    """
    Normalizes the given name. This can be applied to either a models *or* enum.
    """
